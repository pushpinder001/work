  ...
  for (int index = 0; index < (int)m_fm.m_tnum; ++index) {
    for (int j = 0; j < (int)m_fm.m_pos.m_pgrids[index].size(); ++j) {
      if (!m_fm.m_pos.m_pgrids[index][j].empty())
        m_fm.m_pos.m_counts[index][j] = m_fm.m_countThreshold2;				//it value is set to 2 in init function
    }
  }
...
void Cseed::initialMatch(const int index, const int id) {			//function (:
  vector<int> indexes;
  m_fm.m_optim.collectImages(index, indexes);			//images satisfying various constraints such as visData,sequence,angle with index image
  ...
  for (int y = 0; y < gheight; ++y) {
    for (int x = 0; x < gwidth; ++x) {
      ++index2;
      if (!canAdd(index, x, y))									//should we add patch in cell(x,y)
	continue;
	...
	collectCandidates(index, indexes,
                          *m_ppoints[index][index2][p], vcp);	//collecting features for images candidates satisfying consistacy with index 
...
void Cseed::collectCells(const int index0, const int index1,					//cells in index0 with same feature as p0
  ...
  Vec3 line = transpose(F) * point;				//epipole line in other image where cell is projected
...
void Cseed::collectCandidates(const int index, const std::vector<int>& indexes,
  ...
  for (int i = 0; i < (int)indexes.size(); ++i) {        
    ...
    collectCells(index, indexid, point, cells);				//all cells in indexid satisfying a feature point in image index 
    Mat3 F;
    ...
    for (int i = 0; i < (int)cells.size(); ++i) {
      ...
      if (!canAdd(indexid, x, y))							//Should we add patch in cell x,y
	continue;
