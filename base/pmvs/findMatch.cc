...
void CfindMatch::init(const Soption& option) {
  m_timages = option.m_timages;				//all the images in m_timages vector
  m_oimages = option.m_oimages;				//this is empty
  m_images.clear();					//all images that include both of above
  ...
  m_csize = option.m_csize;				//cell size
  m_nccThreshold = option.m_threshold;	//V->V*(photometric consistancy score)
  m_wsize = option.m_wsize;				//For photometric consistacy score b/w two images
  ...
  m_pss.init(m_images, m_prefix, m_level + 3, m_wsize, 1); //photosets library in image folder(m_images vect contain number in name of image)
...
void CfindMatch::run(void) {
  ...
  ++m_depth;
  m_pos.collectPatches();							//path organizer
