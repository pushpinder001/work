...
void Coptim::refinePatchBFGS(Cpatch& patch, const int id,
                             const int time, const int ncc) {		/**********************NCC***********************/
  int status;
...
double Coptim::computeINCC(const Vec4f& coord, const Vec4f& normal,			//normalized cross correlation
...
double Coptim::computeINCC(const Vec4f& coord, const Vec4f& normal,
  ...
  for (int i = 1; i < size; ++i) {
    if (!texs[i].empty()) {
      ...
      if (robust)
        score += rob	ustincc(1.0 - dot(texs[0], texs[i])) * m_weightsT[id][i];
      else
