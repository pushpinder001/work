...
void CpatchOrganizerS::init(void) {
  ...
  for (int index = 0; index < m_fm.m_num; ++index) {			//m_fm.m_num number total images
    const int gwidth = (m_fm.m_pss.getWidth(index, m_fm.m_level)	//m_fm.m_csize is size of patch
                        + m_fm.m_csize - 1) / m_fm.m_csize;			//no of cell in image having m_level and size of cell csize
    const int gheight = (m_fm.m_pss.getHeight(index, m_fm.m_level)
                         + m_fm.m_csize - 1) / m_fm.m_csize;
    m_gwidths[index] = gwidth;						//number of patches in width
    m_gheights[index] = gheight;					//number of patches in height

    if (index < m_fm.m_tnum) {						//m_fm.m_tnum number of target images
      ...
      fill(m_dpgrids[index].begin(), m_dpgrids[index].end(), m_MAXDEPTH);	//filling m_dpgrids with m_MAXDEPTH
    }
  }
}
...
void CpatchOrganizerS::collectPatches(const int target) {			//By default this one is called target=0
  m_ppatches.clear();								//patches in current grid

  for (int index = 0; index < m_fm.m_tnum; ++index) {				//ith image
    for (int i = 0; i < (int)m_pgrids[index].size(); ++i) {			//iterate for total number of pathes in ith image
      vector<Ppatch>::iterator begin = m_pgrids[index][i].begin();
      while (begin != m_pgrids[index][i].end()) {
        (*begin)->m_id = -1;							//id number of patch
        begin++;
      }
    }
  }
  ...
  int count = 0;
  for (int index = 0; index < m_fm.m_tnum; ++index) {				//total number of target images
    for (int i = 0; i < (int)m_pgrids[index].size(); ++i) {
      ...
      while (begin != m_pgrids[index][i].end()) {
        if ((*begin)->m_id == -1) {
          ...
          if (target == 0 || (*begin)->m_fix == 0)				//m_ppatches count is pushed 1 to 60
